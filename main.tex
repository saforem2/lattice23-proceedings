...
\definecolor{myPeriwinkle}{HTML}{8075E6} % {8e82fe}
\definecolor{darkviolet}{rgb}{0.58, 0.0, 0.83}
\definecolor{linkPurple}{HTML}{8A00A6}
\definecolor{sfblue}{HTML}{027FFF}
\definecolor{sfteal}{HTML}{02B386}
\definecolor{sfgreen}{HTML}{44CF6E}  %{36CE5E}  %4CAF50}  %{4BA628}
\definecolor{sfpurple}{HTML}{AB47BD}
...
%  \begin{center}
%    \caption{\label{fig:lattice}Illustration of the lattice\fix{cut to free up space}.}
...
\begin{figure}[h]
    ...
    \end{subfigure}
    %
    \caption{\label{fig:2dU1}Results from trained 2D $U(1)$ model at $\beta = 4$ showing increasing $|\mathcal{J}|$ towards the middle of the trajectory (left), resulting in improved tunneling rate ($\delta Q$) (right).}
\end{figure}
%
...
%\end{equation}
%
To update the gauge field $U_{\mu} = e^{i\omega_{\mu}^{k} \lambda^{k}}$, write,
%
\begin{equation}
...
%
\begin{align}
    \Aoutlineboxed[lfPurple]{\frac{dP^{k}}{dt} = - \frac{\partial H}{\partial \omega^{k}}&} = - \frac{\partial H}{\partial W} = - \frac{dS}{dW} \Longrightarrow \\
...
\end{align}
%
where $F[U] = \frac{dS}{dW}$ is the force term (see~\ref{subsec:force_term}).
%
...
%
In this case, our $\texttt{pNet}: (U, F) = (e^{iW}, F) \rightarrow (\alpha_{P},
\beta_{P}, \gamma_{P})$.
%
